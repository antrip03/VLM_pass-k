"""
A10G (24GB) variant of vram_smoke_test_2000cap.py - same exact
methodology, targeting the cheaper GPU to answer directly: what's the
real minimum GPU memory this plan needs, and does A10G's ~22GB usable
capacity suffice, or is A100 40GB actually required?

Motivation: the A100 run already found training (not generation) is the
binding constraint - 29.32 GB peak at the smallest working micro-batch
(4) there. That alone exceeds A10G's usable capacity, so training at
micro-batch=4 is already known to not fit here. What's NOT yet known
without running this: whether smaller micro-batches (2, 1) fit within
~22GB - extrapolating from the A100 numbers suggests probably yes, but
that is an estimate, not a measurement, and this project's whole
methodology has been to verify rather than trust extrapolation.

Real VRAM smoke test (PLAN.md Step 5): does a 2000-token response cap fit
in memory for both the generation (rollout) phase and the training
(forward+backward+optimizer) phase, on this exact model with the exact
LoRA config intended for training?
...
Usage: modal run scripts/vram_smoke_test_2000cap_a10g.py
"""
...
from configs.modal_app import SMOKE_TEST_GPU, MODEL_CACHE_DIR, MODEL_ID, app, image, model_cache
