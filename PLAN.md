...
6. **Step budget**: **467 steps — REVISED 2026-08-19 from the previous 500 target, and actually executed.** 500 was arithmetically unreachable: GSM8K's train split is 7473 examples and `data.train_batch_size=16` prompts/step, so one epoch is exactly 7473/16 = 467.06 → **467 steps**, and `trainer.total_epochs=1` correctly stops there. Two real runs stopped dead at 467 before this was understood (veRL's end-of-epoch DataLoader teardown surfaces as a misleading `RuntimeError: DataLoader worker ... killed by signal: Killed`, which reads like an OOM but is not). Reaching 500 would have required `total_epochs=2`, making the final 33 steps a second pass over the first ~528 prompts — i.e. ~528 prompts seen twice while ~6,900 are seen once. 467 was chosen deliberately instead: **exactly one epoch, every prompt seen exactly once, no repetition to caveat.** The 500-vs-467 difference is 7% of steps and does not affect the Yue et al. grounding below (their strongest observed effect is at step 450, which 467 clears). Checkpoint every **25** steps (not 100 — tightened because Modal GPUs are preemptible by default with no paid opt-out, so 25 bounds worst-case loss to ~20 min), evaluate text-mode pass@1 at each checkpoint, stop early if plateaued across 2–3 consecutive checkpoints. Directly grounded in the foundational paper for this project (Yue et al. 2504.13837, Table 4, Omni-MATH-Train GRPO): their own step150/300/450 checkpoints show pass@1 still rising (26.1 → 33.6 → 42.5) and pass@256 still declining (66.3 → 65.3 → 64.3, their core sharpening signal) with no plateau through step 450 — 500 lands at/past their point of strongest observed effect. Given this project's smaller batch (S=128 vs. their controlled-experiment 256) increases per-step noise, erring toward more steps rather than fewer compensates somewhat. 300 steps remains a defensible fallback if budget forces a cut (their own step300 checkpoint already shows a real, smaller gain).
...
10. **Training-inference mismatch mitigation** (2026-08-14): `+actor_rollout_ref.rollout.engine_kwargs.vllm.disable_cascade_attn=True` — a real, evidenced fix. vLLM's cascade attention is a shared-prefix batch-decoding optimization; group_size=8 (item 5) means every rollout batch has 8 sequences sharing one prompt, exactly that scenario. A real report (Qwen3-14B-Base, on-policy GRPO, A100s) found this vLLM kernel path produces rollout log-probs that don't exactly match the training engine's for the same tokens, and disabling it dropped their mismatch metric from the 5e-2–1e-1 range to ~1e-3, with training reward also improving — a real quality effect, not just a numerical cleanup. Paired with `actor_rollout_ref.rollout.calculate_log_probs=True`, enabling veRL's own `rollout_corr/*` metrics (kl, k3_kl, chi2_token, chi2_seq, log_ppl_abs_diff, ppl_ratio) in the existing wandb logging, at a real but small documented cost (~1% memory, 1-3% compute). Active importance-sampling correction (`algorithm.rollout_correction.rollout_is`) deliberately left off — watch these metrics during the calibration run (§GCP_SETUP_GUIDE.md / GUIDE.md) and decide from real data whether the residual mismatch after disabling cascade attention actually needs active correction, rather than adding that mechanism preemptively.

**Compute — SUPERSEDED BY REAL MEASUREMENT (2026-08-19).** Training actually ran on Modal (1×A100-40GB, veRL + FSDP2, 467 steps): **≈7.5 hr wall-clock, ≈$22 including all failed/debug runs**, versus the ≈11.1 hr / ~$41 predicted below. Measured per-step timings at S=128, 1200-token cap: gen 15.8s, old_log_prob 7.5s, adv 0.8s, update_actor 15.8s, update_weights 13.8s → **~55s/step**, plus ~41s on each checkpoint-save step. Observed `response_length/mean` stayed ~245 tokens against the 1200 cap, i.e. the cap was never the binding constraint. The pre-run estimate below is retained for provenance only; prefer these real numbers for Phase 2 planning.

**Original (raw-HF-proxy) estimate, retained for provenance (1 seed, 500 steps, S=128 sequences/step, 1200-token cap, A100 40GB, from real Modal-measured throughput at a 2000-token cap - see the note in item 7 above on why these figures remain representative - `scripts/vram_smoke_test_2000cap.py` and `scripts/timing_calibration_2000cap.py`)**: training ≈ 11.1 hr, ~$41 (§9.2 table, S=128 row, 500 steps) + eval ≈ 7.2 hr, ~$26 (§9.3, 50 eval problems, locked) — **total ≈ 18.3 hr, ≈ $67**. This is the now-concrete Phase 1 figure per the locked group size (8) / prompts-per-step (16) / step count (500) / eval-problem-count (50) decisions above; still pending real-veRL calibration (not the raw-HF proxy this is based on) per the optimization discussion above the batch-shape item.
...
- ~~Final LoRA hyperparameters (rank, target modules, learning rate) and Dr. GRPO settings (KL coefficient)~~ — **locked** (2026-08-12, `src/training/phase1_config.py` module docstring for full rationale): `lora_rank=32`, `lora_alpha=32` (real RLVR+LoRA ablation evidence, arXiv 2512.23165: ranks 16/32 plateau in the same accuracy range, r=1 clearly underperforms, and a separate finding that accuracy stagnates/declines above the 32-64 range — no evidence the prior rank=64 default, inherited unverified from a generic veRL example, was actually better); `lr=1e-6`, `use_kl_loss=False` (switched from the same example's unverified 3e-6/kl_loss_coef=0.01 to Dr. GRPO's own real recipe — better-justified independently too: our smaller single-GPU batch argues for the more conservative LR, and nonzero KL would damp Δ_text, the exact quantity this project measures, rather than guard against a real evidenced risk). Target modules were never actually open — vision exclusion is a hard requirement of the design, not a tuning knob.
